...
class MAMEControlConfig(ctk.CTk):
    ...
    def display_game_info(self, rom_name):
        ...
        try:
            ...
            # IMPORTANT: Get the current input mode
            ...
            # IMPORTANT: Always get custom control configuration if it exists
            # and apply mappings with current input mode
            cfg_controls = {}
            if rom_name in self.custom_configs:
                ...
                print(f"Applied custom mapping from ROM CFG for {rom_name} with mode {current_mode}")

            # NEW CODE: Filter out non-XInput controls if in XInput Only mode
            if hasattr(self, 'xinput_only_mode') and self.xinput_only_mode:
                game_data = self.filter_xinput_controls(game_data)
                print(f"Applied XInput filter for display")
...
controller xbox t		= """
    ...
    def update_game_data_with_custom_mappings(self, game_data, cfg_controls):
        ...
        # Get current input mode to ensure proper conversion
        current_mode = getattr(self, 'input_mode', 'xinput' if self.use_xinput else 'joycode')
        print(f"Updating game data with mappings for input mode: {current_mode}")
        ...
        for btn in required_buttons:
            if btn in cfg_controls:
                ...
                if hasattr(self, 'default_controls') and btn in self.default_controls:
                    # UPDATED: Convert to current mode explicitly
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            "input_mode": getattr(self, 'input_mode', 'xinput'),  # Updated to include dinput
    ...
    def load_settings(self):
        ...
        self.use_xinput = True
        self.input_mode = 'xinput'  # Default to xinput
        ...
        if hasattr(self, 'settings_path') and os.path.exists(self.settings_path):
            try:
                ...
                if 'input_mode' in settings:
                    # Validate input mode
                    valid_modes = ['xinput', 'joycode', 'keyboard', 'dinput']
                    self.input_mode = settings['input_mode'] if settings['input_mode'] in valid_modes else 'xinput'
    ...
    def convert_mapping(self, mapping: str, to_mode: str = None) -> str:
        """Convert between JOYCODE, XInput, DirectInput and Keyboard mappings with support for increment/decrement pairs"""
        if not mapping:
            return mapping
        ...
        # For regular mappings, use the single mapping logic
    ...
    def convert_single_mapping(self, mapping: str, to_mode: str) -> str:
        ...
        # Add DirectInput mappings based on PS5 controller and generic DInput standard
        dinput_mappings = {
            # Standard buttons for PS5 controller and similar DirectInput devices
            'JOYCODE_1_BUTTON1': 'DINPUT_1_BUTTON1',      # Square on PS5 / Button 1 on generic
            'JOYCODE_1_BUTTON2': 'DINPUT_1_BUTTON2',      # X on PS5 / Button 2 on generic
            'JOYCODE_1_BUTTON3': 'DINPUT_1_BUTTON3',      # Circle on PS5 / Button 3 on generic
            'JOYCODE_1_BUTTON4': 'DINPUT_1_BUTTON4',      # Triangle on PS5 / Button 4 on generic
            'JOYCODE_1_BUTTON5': 'DINPUT_1_BUTTON5',      # L1 on PS5 / Button 5 on generic
            'JOYCODE_1_BUTTON6': 'DINPUT_1_BUTTON6',      # R1 on PS5 / Button 6 on generic
            'JOYCODE_1_BUTTON7': 'DINPUT_1_BUTTON7',      # L2 on PS5 / Button 7 on generic
            'JOYCODE_1_BUTTON8': 'DINPUT_1_BUTTON8',      # R2 on PS5 / Button 8 on generic
            'JOYCODE_1_BUTTON9': 'DINPUT_1_BUTTON9',      # Create on PS5 / Button 9 on generic
            'JOYCODE_1_BUTTON10': 'DINPUT_1_BUTTON10',    # Options on PS5 / Button 10 on generic
            'JOYCODE_1_BUTTON11': 'DINPUT_1_BUTTON11',    # L3 on PS5 / Button 11 on generic
            'JOYCODE_1_BUTTON12': 'DINPUT_1_BUTTON12',    # R3 on PS5 / Button 12 on generic
            
            # D-pad
            'JOYCODE_1_HATUP': 'DINPUT_1_POV1_UP',        # D-Pad Up
            'JOYCODE_1_HATDOWN': 'DINPUT_1_POV1_DOWN',    # D-Pad Down
            'JOYCODE_1_HATLEFT': 'DINPUT_1_POV1_LEFT',    # D-Pad Left
            'JOYCODE_1_HATRIGHT': 'DINPUT_1_POV1_RIGHT',  # D-Pad Right
            
            # Analog sticks
            'JOYCODE_1_YAXIS_UP_SWITCH': 'DINPUT_1_AXIS2_NEG',    # Left Stick Up
            'JOYCODE_1_YAXIS_DOWN_SWITCH': 'DINPUT_1_AXIS2_POS',  # Left Stick Down
            'JOYCODE_1_XAXIS_LEFT_SWITCH': 'DINPUT_1_AXIS1_NEG',  # Left Stick Left
            'JOYCODE_1_XAXIS_RIGHT_SWITCH': 'DINPUT_1_AXIS1_POS', # Left Stick Right
            
            # Right analog stick
            'JOYCODE_1_RYAXIS_NEG_SWITCH': 'DINPUT_1_AXIS4_NEG',  # Right Stick Up
            'JOYCODE_1_RYAXIS_POS_SWITCH': 'DINPUT_1_AXIS4_POS',  # Right Stick Down
            'JOYCODE_1_RXAXIS_NEG_SWITCH': 'DINPUT_1_AXIS3_NEG',  # Right Stick Left
            'JOYCODE_1_RXAXIS_POS_SWITCH': 'DINPUT_1_AXIS3_POS',  # Right Stick Right
            
            # Player 2 controls (add more as needed)
            'JOYCODE_2_BUTTON1': 'DINPUT_2_BUTTON1',      # P2 Button 1
            'JOYCODE_2_BUTTON2': 'DINPUT_2_BUTTON2',      # P2 Button 2
            'JOYCODE_2_BUTTON3': 'DINPUT_2_BUTTON3',      # P2 Button 3
            'JOYCODE_2_BUTTON4': 'DINPUT_2_BUTTON4',      # P2 Button 4
            'JOYCODE_2_HATUP': 'DINPUT_2_POV1_UP',        # P2 D-Pad Up
            'JOYCODE_2_HATDOWN': 'DINPUT_2_POV1_DOWN',    # P2 D-Pad Down
            'JOYCODE_2_HATLEFT': 'DINPUT_2_POV1_LEFT',    # P2 D-Pad Left
            'JOYCODE_2_HATRIGHT': 'DINPUT_2_POV1_RIGHT',  # P2 D-Pad Right
        }
        
        # Create reverse mappings
        joycode_mappings = {v: k for k, v in xinput_mappings.items()}
        dinput_to_joycode = {v: k for k, v in dinput_mappings.items()}
        
        # Create cross-mappings between XInput and DirectInput
        xinput_to_dinput = {}
        dinput_to_xinput = {}
        
        # Find common JOYCODE mappings that link XInput and DirectInput
        for joy_key, xin_val in xinput_mappings.items():
            if joy_key in dinput_mappings:
                din_val = dinput_mappings[joy_key]
                xinput_to_dinput[xin_val] = din_val
                dinput_to_xinput[din_val] = xin_val
        ...
        if " OR " in mapping:
            ...
            for part in parts:
                ...
                if to_mode == 'xinput' and part.startswith('XINPUT'):
                    ...
                    return part
                elif to_mode == 'dinput' and part.startswith('DINPUT'):
                    print(f"  Found DINPUT part in OR statement: {part}")
                    return part
            
            # If no direct match, try to convert a compatible part
            if to_mode == 'xinput':
                # For xinput mode, try to convert JOYCODE or DINPUT parts
                for part in parts:
                    if part.startswith('JOYCODE') and part in xinput_mappings:
                        ...
                        return converted
                    elif part.startswith('DINPUT') and part in dinput_to_xinput:
                        converted = dinput_to_xinput[part]
                        print(f"  Converting DINPUT->XINPUT: {part} -> {converted}")
                        return converted
            elif to_mode == 'joycode':
                # For joycode mode, try to convert XINPUT or DINPUT parts
                for part in parts:
                    if part.startswith('XINPUT') and part in joycode_mappings:
                        ...
                        return converted
                    elif part.startswith('DINPUT') and part in dinput_to_joycode:
                        converted = dinput_to_joycode[part]
                ...
                for part in parts:
                    if part.startswith('JOYCODE') and part in keyboard_mappings:
                        ...
                        return converted
                    # Add DirectInput to keyboard conversion
                    elif part.startswith('DINPUT'):
                        # Try going through JOYCODE as an intermediate
                        if part in dinput_to_joycode:
                            joycode = dinput_to_joycode[part]
                            if joycode in keyboard_mappings:
                                converted = keyboard_mappings[joycode]
                                print(f"  Converting DINPUT->KEYBOARD via JOYCODE: {part} -> {joycode} -> {converted}")
                                return converted
            elif to_mode == 'dinput':
                # For dinput mode, try to convert from JOYCODE or XINPUT
                for part in parts:
                    if part.startswith('JOYCODE') and part in dinput_mappings:
                        converted = dinput_mappings[part]
                        print(f"  Converting JOYCODE->DINPUT: {part} -> {converted}")
                        return converted
                    elif part.startswith('XINPUT') and part in xinput_to_dinput:
                        converted = xinput_to_dinput[part]
                        print(f"  Converting XINPUT->DINPUT: {part} -> {converted}")
                        return converted
        ...
        if source_mode == 'joycode':
            if to_mode == 'xinput' and mapping in xinput_mappings:
                print(f"  Converting JOYCODE->XINPUT: {mapping} -> {xinput_mappings[mapping]}")
                return xinput_mappings[mapping]
            elif to_mode == 'keyboard' and mapping in keyboard_mappings:
                print(f"  Converting JOYCODE->KEYBOARD: {mapping} -> {keyboard_mappings[mapping]}")
                return keyboard_mappings[mapping]
            elif to_mode == 'dinput' and mapping in dinput_mappings:
                print(f"  Converting JOYCODE->DINPUT: {mapping} -> {dinput_mappings[mapping]}")
                return dinput_mappings[mapping]
        elif source_mode == 'xinput':
            if to_mode == 'joycode' and mapping in joycode_mappings:
                ...
                return keyboard_mappings[mapping]
            elif to_mode == 'dinput' and mapping in xinput_to_dinput:
                print(f"  Converting XINPUT->DINPUT: {mapping} -> {xinput_to_dinput[mapping]}")
                return xinput_to_dinput[mapping]
        elif source_mode == 'keyboard':
            ...
            for joycode, keycode in keyboard_mappings.items():
                if keycode == mapping:
                    if to_mode == 'joycode':
                        ...
                        return joycode
                    elif to_mode == 'xinput' and joycode in xinput_mappings:
                        xinput = xinput_mappings[joycode]
                        print(f"  Converting KEYBOARD->XINPUT: {mapping} -> {joycode} -> {xinput}")
                        return xinput
                    elif to_mode == 'dinput' and joycode in dinput_mappings:
                        dinput = dinput_mappings[joycode]
                        print(f"  Converting KEYBOARD->DINPUT: {mapping} -> {joycode} -> {dinput}")
                        return dinput
        elif source_mode == 'dinput':
            if to_mode == 'joycode' and mapping in dinput_to_joycode:
                print(f"  Converting DINPUT->JOYCODE: {mapping} -> {dinput_to_joycode[mapping]}")
                return dinput_to_joycode[mapping]
            elif to_mode == 'xinput' and mapping in dinput_to_xinput:
                print(f"  Converting DINPUT->XINPUT: {mapping} -> {dinput_to_xinput[mapping]}")
                return dinput_to_xinput[mapping]
            elif to_mode == 'keyboard':
                # Try via joycode
                if mapping in dinput_to_joycode:
                    joycode = dinput_to_joycode[mapping]
                    if joycode in keyboard_mappings:
                        print(f"  Converting DINPUT->KEYBOARD: {mapping} -> {joycode} -> {keyboard_mappings[joycode]}")
                        return keyboard_mappings[joycode]
        
        # Check if it already has parts from the target mode (e.g., SWITCH suffix with JOYCODE)
        if source_mode == 'joycode' and "SWITCH" in mapping:
            for key, value in xinput_mappings.items():
                if key in mapping:
                    ...
                    elif to_mode == 'dinput' and key in dinput_mappings:
                        return dinput_mappings[key]
    ...
    def get_friendly_dinput_name(self, mapping: str) -> str:
        ...
        if not mapping.startswith('DINPUT'):
            return mapping
        ...
        # Extract button or axis information
        if 'BUTTON' in parts[2]:
            button_num = parts[2].replace('BUTTON', '')
            
            # Use the PS5 button names as reference
            ps5_buttons = {
                '1': 'Square',
                '2': 'X',
                '3': 'Circle',
            ...
            # For the UI, we'll display button using generic terms followed by PS5 in parentheses
            if button_num in ps5_buttons:
                return f"Button {button_num} ({ps5_buttons[button_num]})"
            else:
        ...
        # Handle POV (D-pad)
            ...
            return f"D-Pad {direction.capitalize()}"
        
        # Handle axes
        elif 'AXIS' in parts[2]:
            axis_num = parts[2].replace('AXIS', '')
            direction = parts[3] if len(parts) > 3 else ""
            
            # Map axes to common names
            if axis_num == '1':
                return f"Left Stick {'Left' if direction == 'NEG' else 'Right'}"
            elif axis_num == '2':
                return f"Left Stick {'Up' if direction == 'NEG' else 'Down'}"
            elif axis_num == '3':
                return f"Right Stick {'Left' if direction == 'NEG' else 'Right'}"
            elif axis_num == '4':
                return f"Right Stick {'Up' if direction == 'NEG' else 'Down'}"
            elif axis_num == '5' or axis_num == '6':
                return f"Trigger {axis_num}"
            else:
                return f"Axis {axis_num} {direction}"
    ...
    def format_mapping_display(self, mapping: str) -> str:
        ...
        if " OR " in mapping:
            ...
            preferred_part = None
            if current_mode in ('xinput', 'joycode', 'dinput'):
                # Search for mode-specific prefixes first
                prefix_map = {
                    'xinput': 'XINPUT',
                    'joycode': 'JOYCODE',
                    'dinput': 'DINPUT'
                ...
                # Look for exact mode match first
                for part in parts:
                    if part.strip().startswith(prefix_map.get(current_mode, '')):
                        preferred_part = part.strip()
                        break
                
                # If no exact match, fall back to JOYCODE
                if not preferred_part:
                    for part in parts:
                        if "JOYCODE" in part:
                            preferred_part = part.strip()
                            break
    ...
    def toggle_input_mode(self):
        """Cycle between input modes: XInput → JOYCODE → Keyboard → DirectInput → XInput with enforced refresh"""
        ...
        if current_mode == 'xinput':
            ...
            new_mode = 'dinput'  # New DirectInput option
            mode_text = "DirectInput Mode"
            mode_color = "#E86310"  # Orange color for DirectInput
        else:  # 'dinput' or any other value
            new_mode = 'xinput'
        ...
        if current_rom:
            ...
            try:
                ...
                if game_data:
                    ...
                    # IMPORTANT: Force cache file update with the new input mode
                    if hasattr(self, 'cache_dir') and self.cache_dir:
                        cache_path = os.path.join(self.cache_dir, f"{current_rom}_cache.json")
                        # Remove the old cache file first to ensure clean state
                        if os.path.exists(cache_path):
                            try:
                                os.remove(cache_path)
                                print(f"Removed outdated cache file for {current_rom}")
                            except:
                                pass
                        
                        # Save the updated game data to cache file with new mode
                        try:
                            with open(cache_path, 'w', encoding='utf-8') as f:
                                json.dump(game_data, f, indent=2)
                            print(f"Created fresh cache file for {current_rom} with {new_mode} mappings")
                        except Exception as e:
                            print(f"Error creating updated cache: {e}")
    ...
    def display_controls_table(self, start_row, game_data, cfg_controls):
        ...
        if current_mode == 'xinput':
            ...
            mode_text = "JOYCODE Mode"
            mode_color = "#888888"
        elif current_mode == 'dinput':  # Add explicit case for DirectInput
            mode_text = "DirectInput Mode"
            mode_color = "#E86310"  # Orange color for DirectInput
        else:  # keyboard or any other fallback
            mode_text = "Keyboard Mode"
        ...
        try:
            if hasattr(self, 'cache_dir') and self.cache_dir:
                ...
                if hasattr(self, 'xinput_only_mode') and self.xinput_only_mode:
                    ...
                    # Clear any existing cache file to ensure clean state
                    if os.path.exists(cache_path):
                        try:
                            os.remove(cache_path)
                        except:
                    ...
                    # Create new cache file with current input mode
                    with open(cache_path, 'w', encoding='utf-8') as f:
